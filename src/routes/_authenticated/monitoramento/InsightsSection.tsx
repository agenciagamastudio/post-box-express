...
export default function InsightsSection({ insights, loading }: Props) {
  ...
  const totals = insights.reduce(
    (acc, insight) => ({
      impressions: acc.impressions + (insight.impressions as number || 0),
      reach: acc.reach + (insight.reach as number || 0),
      profile_views: acc.profile_views + (insight.profile_views as number || 0),
    }),
        ...
        <Card className="p-4 bg-gradient-to-br from-purple-500/10 to-purple-600/10 border-purple-200 dark:border-purple-800">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{totals.profile_views.toLocaleString()}</p>
            </div>
      ...
      <Card className="p-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={insights}>
            ...
            <Bar dataKey="profile_views" fill="#a855f7" name="Visitas ao Perfil" />
          </BarChart>
        </ResponsiveContainer>
      </Card>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {insights.map((insight) => (
                <tr key={insight.date} className="border-b hover:bg-muted/50">
                  <td className="py-2 px-2">{insight.date}</td>
                  <td className="text-right py-2 px-2">{(insight.impressions as number).toLocaleString()}</td>
                  <td className="text-right py-2 px-2">{(insight.reach as number).toLocaleString()}</td>
                  <td className="text-right py-2 px-2">{(insight.profile_views as number).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
