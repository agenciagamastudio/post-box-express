...
4. *Nome:* `GamaGit` · *e-mail* de contato → **Criar app**.
...
2. Escolha **"Instagram API configurada com login do Instagram"**
   (*Instagram API setup with Instagram login*) — **não** a opção "com login do Facebook".
...
2. Copie:
   ...
   - **Chave secreta do app do Instagram** (*Instagram app secret*) → **IG_APP_SECRET** (clique Mostrar)
...
```
(Se pedir, preencha também *Deauthorize* e *Data deletion* com a mesma base, ex.:
...
1. No produto **Instagram → Funções (Roles) → Testadores do Instagram**
   (*Instagram testers*) → **Adicionar pessoas** → digite seu **@usuário** do Instagram.
